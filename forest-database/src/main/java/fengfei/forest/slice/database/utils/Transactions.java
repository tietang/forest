...
public class Transactions {
	...
	public static void setDatabaseSliceGroupFactory(
			DatabaseRouterFactory databaseRouterFactory, boolean isDev) {
	...
	protected static <T> TaModel<T> retryExecute(TaModel<T> model,
			PoolableDatabaseResource resource,
			final TransactionCallback<T> callback, int tries)
			throws SQLException {
		T t = null;
	...
	protected static <T> TaModel<T> retryExecute(TaModel<T> model,
			DataSource dataSource, final JdbcCallback<T> callback, int tries)
			throws SQLException {
		T t = null;
