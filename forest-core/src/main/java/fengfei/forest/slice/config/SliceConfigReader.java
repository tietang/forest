...
public abstract class SliceConfigReader implements SliceReader<Config> {
	...
	public Config read(String rootPath) throws ConfigException {
		...
		try {
			...
			log.info(String.format("read path: namespace=%s, path=%s",
					namespace, infoPath));
	...
	public <Key, R extends SliceResource> void mapResource(Function function,
			String res, Router<Key, R> router, long sliceId, String alias,
			Range[] ranges) {
