...
public class SliceConfigReader implements SliceReader<Config> {
     ...
     * 
     * <pre>
     * -root_path - routers - router_parent01 - r01 - r02 - root_path - routers - router_parent01 - r01
     *         - r02
     * 
     * 
     * </pre>
    ...
    public Config read(String rootPath) throws ConfigException {
        if (null == rootPath || "".equals(rootPath) || "/".equals(rootPath)
            ...
            throw new ConfigException(String.format(
                "root path is invalid: namespace=%s, path=%s",
                namespace,
                rootPath));
        ...
        try {
            ...
            // defaultExtraInfo
            String infoPath = rootPath + FOREST_EXTRA_INFO_PATH;
            log.info(String.format("read path: namespace=%s, path=%s", namespace, infoPath));
            List<BerainEntry> defaultExtraInfos = source.children(infoPath);
            Map<String, String> kv = toMap(defaultExtraInfos);
            config.defaultExtraInfo.putAll(kv);
            // resources
            String resourcesPath = rootPath + FOREST_RESOURCES_PATH;
            if (source.exists(resourcesPath)) {
                Set<ResConfig> globalResources = readResources(resourcesPath);
                config.setResources(globalResources);
            }
            ...
            for (BerainEntry entry : routerEntries) {
                ...
                Map<String, String> infos = new HashMap<>(config.defaultExtraInfo);
    ...
    void check(String path) throws Exception {
        if (source.exists(path)) {
            ...
            throw new ConfigException(String.format(
                "Non-existed path: namespace=%s, path=%s",
                namespace,
                path));
        }
    }
    ...
    RouterConfig readRouterConfig(BerainEntry entry) throws Exception {
        ...
        //
        routerConfig.resources = readResources(routerConfig.path + FOREST_RESOURCES_PATH);
    ...
    public String getValue(Map<String, String> kv, String routerPath, String path)
        throws Exception {
